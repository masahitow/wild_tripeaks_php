...
$result = mysqli_query($db->conn, "SELECT id, UserID, NickName, GamesPlayed, BestScore, Country, State, created_at FROM scores AS v INNER JOIN (SELECT MAX(id) AS MaxID FROM scores GROUP BY UserID) AS v2 ON v.id = v2.MaxID ORDER BY BestScore DESC, GamesPlayed DESC, id") or die(mysqli_error($db->conn));
