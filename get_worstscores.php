...
$result = mysqli_query($db->conn, "SELECT id, UserID, NickName, GamesPlayed, WorstScore, Country, State, created_at FROM scores AS v INNER JOIN (SELECT MAX(id) AS MaxID FROM scores GROUP BY UserID) AS v2 ON v.id = v2.MaxID ORDER BY WorstScore DESC, GamesPlayed DESC, id") or die(mysqli_error($db->conn));
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_array($result)) {
		...
		if ($counter<=$limit){
			...
			$scores["ScoreValue"] = number_format($row["WorstScore"] * 10);
			$scores["created_at"] = $row["created_at"];
			$scores["Country"] = $row["Country"];
			$scores["State"] = $row["State"];
			$scores["RawData"] = $row["WorstScore"]*10;
